...
export async function createInvoice(preState: State, formData: FormData) {
    const validationForm = CreateInvoice.safeParse({
    ...
    const {customerId, amount,status} = validationForm.data;
...
export async function updateInvoice(  id: string, preState: State, formData: FormData) {
    const validationForm = UpdateInvoice.safeParse({
        customerId: formData.get("customerId"),
        amount: formData.get("amount"),
        status: formData.get("status"),
    });
...
export async function deleteInvoice(id: string){
    ...
    try {
        ...
        `;
        revalidatePath("/dashboard/invoices"); 
        return {message: "Invoices deleted."}
    } catch (error) {
        console.log(error);
        return {
